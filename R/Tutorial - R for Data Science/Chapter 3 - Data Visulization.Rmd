...
This section only talks about prerequisites for data analysis. No notes
needed.
...
We are going to use the MPG data set throughout this chapter. This comes
with the tidyverse package.

A standard graphing template using ggplot is as follows:
...
Doing this I see nothing. I need to define what I want to see using the
grammars of graphics.

***Question 2 - How many rows and columns are in mpg?***

I can find this using the str function, or alternatively glimpse. Or
just writing the data in the console.

```{r}
str(mpg)
glimpse(mpg) # Glimpse is str on crack
mpg # Presents on tibble
```

It is 234 rows and 11 columns.

***Question 3 - what does the dry variable describe?***
...
drv describes whether the car is f (front wheel drive), r (rear wheel
drive), or 4 (4wd)

***Question 4 - make a scatter-plot of hwy vs cyl***

I can do that using geom_point plotting function in ggplot.
...
***Question 5 - What happens if you make a scatter-plot of class vs drv?
...
This is not useful as the type of wheel setup (drv) is not dependent on
type of car. One would expect.

# 3 - Aesthetic mappings

## 3.1 - Notes

Lets you highlight specific stuff. Aesthetics includes things like size,
shape and color. One can use aesthetic mapping like this.
...
Doing this will automatically assign a unique "level" of aesthetic
(color) tp each unique value in the chosen variable (in this case: car
class).
...
This gives us a different aesthetic, however, in this case it looks real
messy. Other things we can map to is: 1. alpha = transparency 2. shape =
shape
...
Answer: Because the deceleration for blue points are inside the x and Y
parentheses, which creates a new variable in MPG called blue, which R
attempts to color.
...
It is somewhat difficult to look at the documentation. It is best to
look at a printed data frame.

```{r}
mpg
```

Here we can see <chr> which stands for categorical and <dbl> or <int>
which stand continuous. dbl is double precision vectors, or double -
from C. I.E. a long float.

**3 - Map a continuous variable to color, size, and shape and look how
they behave compared to categorical values**
...
First it tells me that a continuous variable cant be mapped to shape.
This makes sense.

As for color, it creates a gradient when used with continuous variables.
Size also work with continuous variables, however, is not as good and
rather scales the variables instead of having discrete sizes.
...
facet_wrap() function. There are several arguments you can supply to the
facet function, including: 1. \~ = Formula (i.e. \~ class, nrow = 2) 2.
grid = Two facets (i.e. drv \~ cyl) 3 . = neither rows nor columns (i.e.
. \~ cyl)
...
Empty cells simply means that there are some drv and cyl combinations
that have no values (hence empty). From the graph above I would guess
that drv r with cyl 4 + 5 should be empty. Lets test that.
...
I would guess this would make the opositte, a facet with one row and
alot of columns. The . simply states that you dont want the facets in
any dimensions. You can also use facet_wrap(\~ drv, ncol=1).

**4 - Take this faceted plot. What are the advantages and disadvantages
of using faceting instead of color aesthetic?**
...
Advantages \* You split up your view. Make it simpler if you have a
larger dataset. \* If you have many variables to show, it can be
difficult to see differences in shapes and colour. \* If the cases are
somehow independent of each other.

Disadvantages \* If you just have a small dataset, it can seem empty. \*
Difficult to compare direct values.

**5 - What does nrow and ncol do in facets? And why does not
facet_grid() have a nrow and ncol variable?**
...
Ncol does: number of columns. nrow = number of rows. This is needed at
facet wrap only facets on one variable.

These variables are not needed for facet_grid() as this function lays it
out in rows and columns dependent on the data set.

**6 - When using facet_grid() you should usually put the variable with
more unique levels in the columns, why?**

This is probably because it is easier to read if its longer in the
column compared to rows.

# 5 - Geometric Objects

## 5.1 - Notes

You make different plots/graphs in R by using different geoms (geometric
object). These can include:
...
While all geoms takes mapping functions, they cant contain the same type
of asthetics. For example a line graph is hard pressed to contain
shapes, and different colours deppendent on the class. However, for
smooth geoms you could for example set the line type like this:
...
Here the function seperates the car dependent on the drivetrain type
...
The problem here is however that you esentially write the code two
...
**1 - What geom would you use to draw a line chart, boxplot, hisogram,
or area chart?**
...
I predict that the below code with make a coloured line graph (based on
drv) with points and without the standard error (SE = false).
...
is the diaomonds data set where there is no count variable, however,
this is calculated when using bar graphs. See this.
...
You can actually use stats interchangibally. The previous graph can also
be made like this.
...
There are three reasons you might choose to use stats over geoms. 1.
Overide default stat (i.e. change from count to identity)
...
2.  Overide defailt mapping. You might want to show something other than
    a normal Y column, for example proportion.
...
you rewrite the previous plat using geom instead of stat?**
